...
def plot_train_history(train_hist, y_label, filename, labels=["train"]):
    """
    Plot training and validation history
...
def plot_val_history(val_hist, y_label, filename, labels=["val"]):
    """
    Plot training and validation history
    ...
    #pdb.set_trace()
    # Plot loss and accuracy
    xi = [i for i in range(0, len(val_hist))]
    #plt.plot(train_hist, label = labels[0])
    plt.plot(val_hist, label = labels[0])
    plt.xticks(xi)
    plt.legend()
    plt.xlabel("Epoch")
    plt.ylabel(y_label)
